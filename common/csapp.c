  ...
  while (nleft > 0) {
    if ((nwritten = write(fd, bufp, nleft)) <= 0) {
      if (errno == EINTR) /* interrupted by sig handler return */
        ...
        break; /*ignore error when fd closed*/
      else
...
int Pipe(int pipefd[2]) {
  int rc;
  if ((rc = pipe(pipefd)) == -1) unix_error("Pipe error");
  return rc;
}
...
/*==================================================
=            Wrapper for epoll function            =
==================================================*/
int Epoll_create1(int flags)
{
  int rc;
  if((rc = epoll_create1(flags)) == -1)
...
{
  int rc;
  if((rc = epoll_ctl(epfd,op,fd,event)) == -1)
...
/*=====  End of Wrapper for epoll function  ======*/
